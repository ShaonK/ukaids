...
export default function GenerationHistoryClient() {
    ...
    async function load() {
        try {
            ...
            setIncome(data.income || { total: 0, levels: {}, fromUsers: {} });
            setDirects(data.directReferrals || 0);
        } catch (e) {
            console.error("LOAD GENERATION TREE ERROR:", e);
        }
    }
    ...
    const Node = ({ node }) => {
        ...
        const rateDisplay = LEVEL_RATES[node.generation]
            ? LEVEL_RATES[node.generation] * 100 + "%"
            : "0%";
            ...
            <div className="ml-4 border-l border-gray-700 pl-3 mt-2">
                ...
                >
                    ...
                    <div>
                        ...
                        {levelUnlocked && gotIncome ? (
                            <p className="text-xs text-green-400">
                                Income: {gotIncome.toFixed(2)}
                            </p>
                        ) : (
                            <p className="text-xs text-red-400">
                                Income: 0.00 — Level {node.generation} Locked.
                                Needs {requiredDirect} directs.
                            </p>
                        )}
                    </div>
                </div>
