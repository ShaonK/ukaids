  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch("/api/user/recharge", {
        ...
        body: JSON.stringify({
          amount: amount.trim(),   // ✅ STRING ONLY
          trxId: trx.trim(),
        }),
      });
    ...
    <div className="p-4 flex flex-col items-center text-white">
      ...
      <div className="w-[340px] bg-[#121212] border border-gray-800 rounded-xl p-5 space-y-4">
        ...
        <div className="bg-[#1A1A1A] border border-orange-600/40 rounded-lg px-3 py-2 text-center">
          <p className="text-xs text-gray-400">Send only via network</p>
          <p className="text-sm font-semibold text-orange-400">{NETWORK}</p>
        </div>
        ...
        <form className="space-y-3" onSubmit={handleSubmit}>
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-lg bg-[#0F0F0F] text-white border border-gray-700"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-2 py-2 rounded-lg font-semibold bg-gradient-to-r from-[#EC7B03] to-[#FF9F1C] text-black disabled:opacity-60"
          >
