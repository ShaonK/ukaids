...
export default function AdminDepositsPage() {
  ...
  return (
    <div className="p-4 text-white">
      ...
      <h2 className="text-xl font-bold text-center mb-4">Pending Deposits</h2>

      {loading && <p className="text-center text-gray-400">Loading...</p>}

      {!loading && pending.length === 0 && (
        <p className="text-center text-gray-400">No pending deposits found.</p>
      )}
        ...
        {pending.map((item) => (
          ...
          >
            <div className="flex justify-between">
              <p className="text-sm font-semibold">User ID: {item.userId}</p>
              <span className="text-yellow-400 text-xs">Pending</span>
            </div>

            <p className="text-xs text-gray-300">Amount: {item.amount}</p>
            <p className="text-xs text-gray-400">TRX: {item.trxId}</p>
